...
CREATE TABLE IF NOT EXISTS `regalos`.`RECIBE` (
  ...
  PRIMARY KEY (`Día`, `REGALO_Numero_referencia`),
...
CREATE TABLE IF NOT EXISTS `regalos`.`RECEPTOR` (
  ...
  INDEX `fk_RECEPTOR_RECIBE1_idx` (`RECIBE_Día` ASC, `RECIBE_REGALO_Numero_referencia` ASC),
  ...
  CONSTRAINT `fk_RECEPTOR_RECIBE1`
    FOREIGN KEY (`RECIBE_Día` , `RECIBE_REGALO_Numero_referencia`)
    REFERENCES `regalos`.`RECIBE` (`Día` , `REGALO_Numero_referencia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
