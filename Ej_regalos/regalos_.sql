...
CREATE TABLE IF NOT EXISTS `regalos`.`PREFERENCIA` (
  ...
  INDEX `fk_PREFERENCIA_RECEPTOR1_idx` (`RECEPTOR_Id_receptor` ASC),
  ...
  CONSTRAINT `fk_PREFERENCIA_RECEPTOR1`
    FOREIGN KEY (`RECEPTOR_Id_receptor`)
    REFERENCES `regalos`.`RECEPTOR` (`Id_receptor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
DELIMITER $$
USE `regalos`$$
CREATE DEFINER = CURRENT_USER TRIGGER `regalos`.`REGALO_BEFORE_INSERT` BEFORE INSERT ON `REGALO` FOR EACH ROW
BEGIN  
